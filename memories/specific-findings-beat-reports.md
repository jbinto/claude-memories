...
**In exploratory and analysis work, every finding must be specific: the file, the timestamp, the number, what's surprising.** Multiple independent signals agreeing on one concrete moment is a finding; a model emitting a corpus-level aggregate is not. Lead with the most specific identifiable instance ("file X at t=85:14, confirmed by two other signals" — not "the dataset shows high entropy overall"), and skip trailing summary paragraphs that restate the find abstractly. The data IS the summary.
