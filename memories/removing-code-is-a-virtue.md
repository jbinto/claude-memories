...
**Defaults:** when you catch yourself adding guards/flags/state for reliability, ask whether a subtractive design meets the real requirement. For a well-solved problem, a battle-tested, popular, no-controversy library beats hand-rolled code you must maintain. Surface deletion candidates to the human rather than silently landing them — deletion is where a human check matters most. The same gradient applies to docs, issues, and prose: content that no longer earns its read is a delete candidate, not an archive candidate.
