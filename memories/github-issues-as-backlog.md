...
2. **Same-breath body sync** — any comment that changes an issue's truth (a ruling, scope change, supersession) gets the body edited in the same breath, linking the comment. Readers skim bodies; a body contradicted by its own thread silently misleads them.
...
4. **Sweeps declare bankruptcy** — reconciling an issue with layered history ends in a NEW issue carrying only the distilled end-state (rulings with dates, build spec, provenance link), closing the old with a pointer. Never leave a repaired-in-place body as the build source.
