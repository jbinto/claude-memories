...
- [drive-unattended-runs](memories/drive-unattended-runs.md) — in autonomous runs the enemy is STOPPING (not narration): sustain one push, bias to action on authorized paths, wait actively with a fallback timeout, plan multiple tracks, pivot on blockers.
