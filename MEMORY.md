...
- [re-present-defaulted-decisions](memories/re-present-defaulted-decisions.md) — WHEN the agent takes decisions on the user's behalf by default ("going with X unless you object"): each is a debt — re-present it concretely at landing with the tradeoff, still flippable. (Interaction hygiene, not code/schema defaults.)
